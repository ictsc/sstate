...
# 設定ファイルを読み込み、指定された problem_id に基づいて node_name を取得
def load_config(problem_id):
    ...
    for problem in config.get("common_config", {}).get("problems", []):
        if problem.get("problem_id") == problem_id:
            return problem.get("node_name")

    print(f"設定ファイル内で Problem ID {problem_id} が見つかりませんでした。")
    sys.exit(1)

if len(sys.argv) < 2:
    print("エラー: VM ID が引数として必要です。")
    print("使用法: python proxmox_vm_config_fetcher.py <vm_id>")
    sys.exit(1)

# コマンドライン引数で渡された完全な VM ID（例: 1000101）
vm_id = sys.argv[1]

# Problem ID を抽出し、node_name を取得
problem_id = vm_id[3:5]  # '100{problem_id}XX' の形式から problem_id を抽出
node_name = load_config(problem_id)
